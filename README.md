## Hi there 👋

I'm **Julach Earzan**, an graduated at Eastern University, Sri Lanka, pursuing a degree in Computer Science. I have a passion for developing responsive web applications and exploring various programming paradigms.

### 🔭 Projects and Repositories

Here are some of the notable projects I've worked on:

- **[My Portfolio](https://github.com/julach-erazan/portfolio-2)**: A responsive web page built using HTML, CSS, and JavaScript to showcase my skills and projects.

- **[FoodShop Website](https://github.com/julach-erazan/FoodShop_website)**: An e-commerce platform developed with React.js and MySQL, facilitating seamless online food ordering experiences.

- **[Python Game](https://github.com/julach-erazan/python-game)**: A classic snake game implemented in Python, demonstrating fundamental game development concepts.

- **[PDF to DOCX Converter](https://github.com/julach-erazan/pdf_to_docx)**: A web application that converts PDF files to DOCX format using HTML, CSS, PyScript, and Python.

- **[Student Management System](https://github.com/julach-erazan/Student-Management-System-MERN-)**: A comprehensive system built with the MERN stack (MongoDB, Express.js, React.js, Node.js) to manage student data efficiently.

### 🌱 Currently Learning

I'm expanding my knowledge in the following areas:

- **Machine Learning**: Delving into algorithms and models to enable systems to learn and make decisions.

- **Backend Development**: Enhancing my skills in server-side technologies and database management.

### 👯 Collaboration

I'm open to collaborating on projects that involve:

- Web Development (Frontend and Backend)

- Machine Learning Applications

- Open Source Contributions

### 📫 How to Reach Me

Feel free to connect with me via:

- **Email**: [julach99214@gmail.com](mailto:julach99214@gmail.com)

- **LinkedIn**: [linkedin.com/in/julach-earzan](https://www.linkedin.com/in/julach-earzan)

### ⚡ Fun Fact

When I'm not coding, I enjoy exploring the latest tech trends and playing strategic video games.

